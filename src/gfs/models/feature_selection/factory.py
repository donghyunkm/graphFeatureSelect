...
def get_feature_selector(fs_method: str, n_genes: int, n_select: int, **kwargs) -> FeatureSelector:
    if fs_method == "gumbel":
        ...
        return ScGistFeatureSelector(n_genes=n_genes, n_select=n_select, l1=kwargs.get("l1", 0.1))
    elif fs_method == "stg":
        return STGFeatureSelector(n_genes=n_genes, n_select=n_select, sigma=kwargs.get("sigma", 0.5))
    else:
