...
from gfs.MPNN import MPNNs
from gfs.MPNN_concrete import MPNN_conc
...
class GNN(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_macro_acc", train_macro_acc, on_step=False, on_epoch=True, prog_bar=True, logger=True, batch_size=1
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "val_overall_acc", val_overall_acc, on_step=False, on_epoch=True, prog_bar=True, logger=True, batch_size=1
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.halfhop:
            ...
            celltype_pred = self.forward(
                gene_exp, edgelist, xyz, exp_decay_temp_schedule(epoch, self.trainer.max_epochs), False
            )
        else:
            transform = T.HalfHop(alpha=0.5)
            data = transform(data)
