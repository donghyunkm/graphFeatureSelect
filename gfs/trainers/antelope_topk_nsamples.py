...
@hydra.main(config_path="../configs", config_name="antelope_topk", version_base="1.2")
def main(config: DictConfig):
    ...
    # paths
    paths = get_paths()
    expname_config = f"{config.data.prefix}_s{config.data.rand_seed}_f{config.data.cv}_topk_{config.topk.sampler}_ppssampler_{config.topk.pps_sample}_ppsact_{config.topk.pps_activation}_ppsgrad_{config.topk.pps_gradient}_subgraphmode_{config.topk.subgraph}_"

    # expname = get_datetime(expname=config.expname)
    expname = get_datetime(expname=expname_config)
    print(expname_config)
    print(expname)
