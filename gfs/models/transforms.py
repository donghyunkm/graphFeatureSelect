...
class HalfHop(BaseTransform):
    ...
    def __init__(self, alpha: float = 0.5, p: float = 1.0) -> None:
        if alpha < 0.0 or alpha > 1.0:
            raise ValueError(f"Interpolation factor has to be between 0 and 1 (got '{alpha}'")
        if p < 0.0 or p > 1.0:
            raise ValueError(f"Ratio of half-hopped edges has to be between 0 and 1 (got '{p}'")
    ...
    def forward(self, data: Data) -> Data:
        ...
        slow_node_ids = torch.arange(num_halfhop_edges, device=x.device) + num_nodes
