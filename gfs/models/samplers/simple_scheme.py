...
from gfs.models.samplers.simple_pkg.simple import Layer
    ...
    def forward(self, scores, sample_from_score=True, tau=1.0):
        ...
        flat_scores = torch.cat(
            [
                flat_scores,
                torch.full(
                    (flat_scores.shape[0], N - flat_scores.shape[1]),
        ...
        if sample_from_score:
            ...
            samples = samples.reshape(
                self.n_samples, nnodes, ensemble, choices
            ).permute((0, 1, 3, 2))
            samples = (samples - marginals[None]).detach() + marginals[None]
            if self.assign_value:
                samples = samples * (marginals[None] + 1.0e-7)
        else:
            samples = None
