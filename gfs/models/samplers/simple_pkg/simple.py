...
torch._dynamo.reset()
from gfs.models.samplers.simple_pkg.create_simple_constraint import create_and_save
...
def levelwiseMars(
    ...
    for level in reversed(levels):
        data[level] = (
            theta[parents[level].unbind(-1)] + data[parents[level].unbind(-1)[0]]
        ).logsumexp(-2)
...
def log1mexp(x):
    ...
    minus_x = -x.abs()
    rel_x = torch.where(minus_x == 0.0, -1.0e-10, minus_x)
    new_x = torch.where(
        rel_x > -0.6931471805599453094,
        torch.log(-torch.expm1(rel_x)),
        torch.log1p(-torch.exp(rel_x)),
    )
    return new_x
...
class Layer:
    def __init__(self, n, k, device, root="./simple_configs"):
        ...
        parents = torch.empty(
            (id, max_parents, 2), dtype=torch.int, device=device
        ).fill_(id)
        for k, v in parents_dict.items():
            parents[k] = torch.tensor(
                v + [[id, 0]] * (max_parents - len(v)), dtype=torch.int, device=device
            )  # .to(device)
        ...
        for level in levels_nodes:
            levels.append(
                torch.tensor([l.id for l in level], dtype=torch.int, device=device)
            )
        levels.reverse()
        self.levels = levels
        ...
        idx2primesub = torch.zeros(
            (id, max_elements, 2), dtype=torch.int, device=device
        )
        for node in nodes:
            if node.is_decomposition():
                tmp = torch.tensor(
                    [[p.id, s.id] for p, s in node.elements], dtype=torch.int
                )
                idx2primesub[node.id] = torch.nn.functional.pad(
                    tmp, (0, 0, 0, max_elements - len(tmp)), value=id
                )
    ...
    def log_pr(self, log_probs):
        ...
        levelwiseMars(
            [self.literal_indices] + self.levels[:-1],
            self.idx2primesub,
            data,
            theta,
            self.parents,
