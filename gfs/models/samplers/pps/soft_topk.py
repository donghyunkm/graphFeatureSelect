"""Differentiable Top-k operator via Optimal Transport. (Xie et al. 2020)"""
...
def sinkhorn_forward_stabilized(C, mu, nu, epsilon, max_iter):
    bs, n, k_ = C.size()
    k = k_ - 1
...
class TopKFunc(Function):
        ...
        with torch.no_grad():
            if epsilon > 1e-2:
                ...
                if bool(torch.any(Gamma != Gamma)):
                    ...
                    Gamma = sinkhorn_forward_stabilized(C, mu, nu, epsilon, max_iter)
            else:
                Gamma = sinkhorn_forward_stabilized(C, mu, nu, epsilon, max_iter)
    ...
    def backward(ctx, grad_output_Gamma):
        ...
        mu, nu, Gamma = (
            ctx.saved_tensors
        )  # mu [1, n, 1], nu [1, 1, k+1], Gamma [bs, n, k+1]
        with torch.no_grad():
...
class TopK_custom(torch.nn.Module):
    ...
    def forward(self, scores):
        ...
        # replace -inf with a safe minimum value
...
def soft_topk(x, k):
    k = int(k if isinstance(k, (int, float)) else k[0])
    return TopK_custom(k)(x)[0].sum(dim=-1)


if __name__ == "__main__":
    # Example usage
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    theta = torch.tensor([-10, 0, 5, 10], device=device).float().unsqueeze(0)
    p = soft_topk(theta, k=3)
    print(theta)
    print(p)
